...
{
    public partial class TeaLog : Form
    {
        ...
        bool Authenticate()
        {
            ...
            if (string.IsNullOrWhiteSpace(fidbox.Text) ||
        ...
        private void logbtn_Click(object sender, EventArgs e)
        {
            if (!Authenticate())
            {
                MessageBox.Show("Please enter both Faculty ID and Password.", "Login Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            string query = "SELECT COUNT(1) FROM Faculty WHERE Id = @Id AND Password = @Password";
            ...
            try
            {
                ...
                if (count == 1)
                {
                    ...
                    TeaLog.CurrentUserRole = "Faculty";
                ...
                {
                    MessageBox.Show("Invalid Faculty ID or Password.", "Login Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
